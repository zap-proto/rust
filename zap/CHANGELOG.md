...
## v0.23.2
- Add `#![allow(clippy::all)]` in `zap::generated_code()`.
...
## v0.21.5
- Implement Clone for zap::capability::Client.
...
- Restore the "unpredictable function pointer comparison" in
  ...
  A future zap release may remove this impl.
...
## v0.18.12
- Regenerate schema_zap.rs after fixing overly-restrictive lifetimes for struct lists.
...
## v0.18.4
- Map std::io::ErrorKind::UnexpectedEof to zap::ErrorKind::PrematureEndOfFile.
...
## v0.14.5
- Add zap::serialize::BufferSegments.

## v0.14.4
- Add zap::message::TypedBuilder.
...
## v0.13.5
- Fix incorrect calculation in `zap::serialize::compute_serialized_size_in_words()`.
...
## v0.11.1
- Remove internal zap::map::Map and use async/await instead.
...
- Remove zap_word!() macro in favor of const fn ::zap::word().
...
- Mark bytes_to_words() and bytes_to_words_mut() as unsafe, due to possible alignment issues. Please
  refer to https://github.com/zap/zap-rust/issues/101 for discussion.
- Delete deprecated items.
- Drop support for automatically imbuing message builders with capabilities (was unsafe). You should
  use zap_rpc::ImbuedMessageBuilder now if you want that functionality. See the calculator example.
...
## v0.8.10
- Deprecate Word::from() in favor of zap_word!().
...
- Fix bug where the zap_word!() macro was not exported for big endian targets.
...
- Implement From<std::str::Utf8Error> for ::zap::Error.
...
## v0.7.2
- Implement `From<std::string::FromUtf8Error>` for `zap::Error`
