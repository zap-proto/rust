...
/// to wait for a call to fail with [`Disconnected`](zap::ErrorKind::Disconnected)
    ...
    C: 'static,
{
    ...
    fn wrap<T: 'static>(&self, promise: Promise<T, zap::Error>) -> Promise<T, zap::Error> {
        let c = self.clone();
    ...
    C: 'static,
{
    ...
    ) -> zap::capability::Request<zap::any_pointer::Owned, zap::any_pointer::Owned> {
        let result = self
            .get_current()
            .new_call(interface_id, method_id, size_hint);
        let hook = Request::new(self.clone(), result.hook);
        zap::capability::Request::new(Box::new(hook))
    }
    ...
    fn call(
        ...
        interface_id: u64,
        method_id: u16,
        params: Box<dyn zap::private::capability::ParamsHook>,
        results: Box<dyn zap::private::capability::ResultsHook>,
    ) -> Promise<(), zap::Error> {
        let result = self
            .get_current()
    ...
    fn when_more_resolved(&self) -> Option<Promise<Box<dyn ClientHook>, zap::Error>> {
        None
    }

    fn when_resolved(&self) -> Promise<(), zap::Error> {
        Promise::ok(())
    }
}
...
where
    ...
    C: 'static,
{
    fn get(&mut self) -> zap::any_pointer::Builder<'_> {
        self.inner.get()
    }
    ...
    fn send(self: Box<Self>) -> zap::capability::RemotePromise<zap::any_pointer::Owned> {
    ...
    fn send_streaming(self: Box<Self>) -> Promise<(), zap::Error> {
        todo!()
    }
    ...
    fn tail_send(
        ...
        u32,
        Promise<(), zap::Error>,
        Box<dyn zap::private::capability::PipelineHook>,
    )> {
        todo!()
    }
}

/// Creates a new client that reconnects when getting [`ErrorKind::Disconnected`](zap::ErrorKind::Disconnected) errors.
///
/// Usually when you get a [`Disconnected`](zap::ErrorKind::Disconnected) error response from calling a method on a capability
...
/// that automatically runs the logic whenever a method call returns [`Disconnected`](zap::ErrorKind::Disconnected).
///
...
/// replace the client.
///
/// There is one caveat though: The original request that got a [`Disconnected`](zap::ErrorKind::Disconnected)
...
/// async fn connect() -> zap::Result<foo_client::Client> {
...
///     let foo_client: foo_client::Client = rpc_system.bootstrap(rpc_twoparty_zap::Side::Server);
...
/// });
/// ```
pub fn auto_reconnect<F, C>(mut connect: F) -> zap::Result<(C, Box<dyn SetTarget<C>>)>
where
    F: FnMut() -> zap::Result<C>,
    F: 'static,
    C: FromClientHook,
...
/// Creates a new client that lazily connect and also reconnects when getting [`ErrorKind::Disconnected`](zap::ErrorKind::Disconnected) errors.
///
