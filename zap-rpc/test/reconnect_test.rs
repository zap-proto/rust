...
struct TestInterfaceInner {
    error: Option<Error>,
    generation: usize,
    block: Option<Shared<Promise<(), zap::Error>>>,
}
...
impl TestInterfaceImpl {
    ...
    fn set_error(&self, err: zap::Error) {
        self.inner.borrow_mut().error = Some(err);
    }

    fn block(&self) -> oneshot::Sender<zap::Result<()>> {
        let (s, r) = oneshot::channel();
        self.inner.borrow_mut().block = Some(
            Promise::from_future(r.map(|ret| match ret {
                Ok(Ok(_)) => Ok(()),
                Ok(Err(err)) => Err(err),
                Err(_) => Err(zap::Error::failed("dropped sender".into())),
            }))
            .shared(),
        );
        s
...
where
    F: Future<Output = zap::Result<Response<test_interface::foo_results::Owned>>>,
{
...
fn do_autoconnect_test<F>(pool: &mut LocalPool, wrap_client: F) -> zap::Result<()>
where
    F: Fn(test_interface::Client) -> test_interface::Client,
{
    ...
    let (req3, fulfiller, promise1, promise2, promise4) = {
        ...
        current_server
            .borrow()
            .set_error(zap::Error::disconnected("test1 disconnect".into()));
        assert_err!(
...

        {
            ...
            fulfiller
                .send(Err(zap::Error::disconnected("test2 disconnect".into())))
                .unwrap();
            assert_err!(
                run_until(pool, promise1).expect_err("disconnect error"),
                zap::Error::disconnected("test2 disconnect".into())
            );
            assert_err!(
        ...
        current_server
            .borrow()
            .set_error(zap::Error::disconnected("test3 disconnect".into()));
    ...
    assert_err!(
        ...
        zap::Error::disconnected("test3 disconnect".into())
    );
    ...
    match run_until(pool, req3.send().promise) {
        ...
        Err(err) => {
            assert_err!(err, zap::Error::disconnected("test3 disconnect".into()));
        }
    }
...
impl test_zap::bootstrap::Server for Bootstrap {
    async fn test_interface(
        self: Rc<Self>,
        _params: test_zap::bootstrap::TestInterfaceParams,
        mut results: test_zap::bootstrap::TestInterfaceResults,
    ) -> Result<(), Error> {
...
fn auto_reconnect_rpc_call() {
    ...
    let b = Bootstrap::new();
    let bootstrap: test_zap::bootstrap::Client = zap_rpc::new_client(b.clone());
