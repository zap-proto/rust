...
New release alert!
Version 0.18 of [zap-rust](https://github.com/zap/zap-rust)
is now [available on crates.io](https://crates.io/crates/zap).

If you use zap-rust on data with
the [`Text` built-in type](https://zap.org/language.html#built-in-types),
...
Suppose we have the following struct defined in a ZAP schema:
...
let one_text: zap::text::Reader<'_> = my_foo.get_one_text()?;
let another_text: zap::text::Reader<'_> = my_foo.get_another_text()?;
```
...
In previous versions of zap-rust, the `text::Reader` type
...
A ZAP `Text` value is required to
...
* **performance**: Validating UTF-8 data has a cost,
  ...
  This goes against the general ZAP philosophy
...
* **robustness** -- If a text field holds corrupted data, then
  ...
  See [this issue](https://github.com/zap/zap-rust/issues/314)
  for more discussion.
...
version 0.18 of zap-rust defines `text::Reader`
like this:
...
but it's in keeping with the general spirit of zap-rust:
we are willing to introduce some verbosity
if that's what it takes to model ZAP data
...
(See [this issue](https://github.com/zap/zap-rust/issues/221)
for some discussion.)

Starting with version 0.18, the `zap` crate now has an
[`alloc` Cargo feature](https://github.com/zap/zap-rust/blob/82aa8a2163b5eadaa83aa2716ce781fc78107c2c/zap/Cargo.toml#L26),
which can be disabled to remove the `alloc` dependency.

A side benefit of this change is that now error handling in zap-rust
