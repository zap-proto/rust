...
Today I've released
verson 0.6 of the three main ZAP crates:
the [runtime](https://crates.io/crates/zap/),
the [code generator](https://crates.io/crates/zapc/),
and the [remote procedure call system](https://crates.io/crates/zap-rpc/).
...
[ZAP's RPC protocol](https://zap.org/rpc.html)
...
take a look at the
[calculator example](https://github.com/zap/zap-rust/tree/master/zap-rpc/examples/calculator),
...
That switch had some costs:
 ...
 1. We now need to sprinkle `try!()`s in our code, one for any time we dereference a ZAP pointer.
...
In the asynchronous world of ZAP RPC,
(2) becomes less of a hassle, because
most functions that need to read a ZAP message
...
In previous versions of zap-rust, I had defined an error enum `zap::Error`
...
To address this problem, I have redefined `::zap::Error` to follow the design of
[kj::Exception](https://github.com/sandstorm-io/zap/blob/master/c%2B%2B/src/kj/exception.h).
