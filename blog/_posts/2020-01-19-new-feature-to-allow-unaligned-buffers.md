...
how zap-rust might relax its memory alignment requirements
and what the performance cost of that might look like.
The [ensuing discussion](https://www.reddit.com/r/rust/comments/en9fmn/should_zaprust_force_users_to_worry_about/)
taught me that memory alignment issues can be thornier than I had thought,
and it strengthened my belief that zap-rust users ought be shielded
...
Today I'm releasing it as part of zap-rust version 0.12.
...
As of version 0.12, the `zap` crate has a new
[feature flag called `unaligned`](https://github.com/zap/zap-rust/blob/9fa83b89eebb10aba6a1181bb7e4f9a4fad916f6/zap/Cargo.toml#L30-L32).
When `unaligned` is enabled, `zap` makes no assumptions about the alignment of its data.
In particular, it can read a message in place from any array of bytes via
[`read_message_from_flat_slice()`](https://github.com/zap/zap-rust/blob/9fa83b89eebb10aba6a1181bb7e4f9a4fad916f6/zap/src/serialize.rs#L57-L65).

On the flip side, when `unaligned` is *not* enabled, `zap` requires that message segments are 8-byte aligned,
[returning an error](https://github.com/zap/zap-rust/blob/9fa83b89eebb10aba6a1181bb7e4f9a4fad916f6/zap/src/private/arena.rs#L92-L100)
if it detects that's not the case.
The 8-byte alignment is then used whenever
`zap` loads or stores a primitive value in a message.
...
I ran zap-rust's
[benchmark suite](https://github.com/zap/zap-rust/tree/9fa83b89eebb10aba6a1181bb7e4f9a4fad916f6/benchmark)
on three different computers: my laptop (x86_64), an EC2 ARM64 instance (aarch64), and a Raspberry Pi Zero (armv6).
I compared three different zap-rust versions: 0.11, 0.12, and 0.12 with `unaligned`.
...
Following [ralfj's suggestion](https://www.reddit.com/r/rust/comments/en9fmn/should_zaprust_force_users_to_worry_about/fedr67j/),
...
I added [some tests](https://github.com/zap/zap-rust/blob/9fa83b89eebb10aba6a1181bb7e4f9a4fad916f6/zap/src/private/layout_test.rs#L24)
