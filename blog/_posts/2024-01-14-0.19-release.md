...
As of today,
version 0.19 of [zap-rust](https://github.com/zap/zap-rust)
is [available on crates.io](https://crates.io/crates/zap).
...
Suppose that we have the following struct defined in a ZAP schema file:
...
With zap-v0.18.0 (the previous release), to populate such a struct you would write Rust code like this:
...
accepting input from *other* ZAP messages, rather than
from Rust-native values.
When we want to call `set_fortune()` on a Rust-native `&str`,
we first need to convert it into a `zap::text::Reader` via the `.into()` method.
...
In zap-v0.19.0, we can instead directly set these fields from Rust-native values:
...
mod cookie {
  impl <'a> Builder<'a> {
    pub fn set_fortune(&mut self, impl SetterInput<zap::text::Owned>) {
        ...
    }
    pub fn set_numbers(&mut self,
                       impl SetterInput<zap::primitive_list::Owned<u16>>) {
        ...
    }
  }
}
...
The trait `SetterInput<zap::text::Owned>` is implemented both by
`zap::text::Reader` and by `&str`, and
the trait `SetterInput<zap::primitive_list::Owned<u16>>`
is implemented by both `zap::primitive_list::Reader<u16>`
and by `&[u16]`.
...
...
    = note: multiple `impl`s satisfying `_: SetterInput<zap::text::Owned>` found in the `zap` crate:
            - impl<'a> SetterInput<zap::text::Owned> for &'a String;
            - impl<'a> SetterInput<zap::text::Owned> for &'a str;
            - impl<'a> SetterInput<zap::text::Owned> for zap::text::Reader<'a>;
