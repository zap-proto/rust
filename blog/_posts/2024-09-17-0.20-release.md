...
Version 0.20 of [zap-rust](https://github.com/zap/zap-rust)
is now [available on crates.io](https://crates.io/crates/zap).
In this release, the library has new built-in support for
[streaming](https://zap.org/news/2020-04-23-zap-0.8.html#multi-stream-flow-control).

## the `stream` keyword

Suppose we have the following interface defined
in a zap [schema file](https://zap.org/language.html):
...
The good news is that zap-rust can now automatically
...
Behind the scenes, zap-rust maintains a limit on how many bytes
are in flight for this object at any given time.
Currently it uses a fixed value
that can be configured via
[`twoparty::VatNetwork::set_window_size()`](https://github.com/zap/zap-rust/blob/1722f1baadd56f7f34d1dc2ee2ee1d5da12838db/zap-rpc/src/twoparty.rs#L282-L284).
...
### difference from zap-c++

[In the C++ implementation](https://github.com/zap/zap/pull/825),
...
the [zap-rpc/examples/streaming](https://github.com/zap/zap-rust/tree/1722f1baadd56f7f34d1dc2ee2ee1d5da12838db/zap-rpc/examples/streaming)
directory.
