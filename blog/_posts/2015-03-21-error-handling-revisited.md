...
switch [zap-rust](https://www.github.com/dwrensha/zap-rust)
...
input returns a `::std::result::Result<T, ::zap::Error>`
instead of a bare `T`.
...
There is `::zap::Error`, which gets returned
when a malformed pointer field is encountered in the encoded message.
There is also `::zap::NotInSchema`, which indicates that
...
Because `::zap::Error` implements `::std::error::FromError<::zap::NotInSchema>`,
