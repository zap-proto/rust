...
Today I am releasing version 0.15.0 of the zap-rust crates:
[`zap`](https://crates.io/crates/zap),
[`zapc`](https://crates.io/crates/zapc),
[`zap-futures`](https://crates.io/crates/zap-futures),
and [`zap-rpc`](https://crates.io/crates/zap-rpc).
...
Say we have a ZAP [schema](https://zap.org/language.html)
...
When we pass this to the `zapc-rust` code generator, it
...
allowing us to
read a zap-encoded `Foo` value directly from a byte buffer without
...
a generic container type for a ZAP message,
...
```rust
impl zap::traits::Owned for foo::Owned {
    type Reader<'a>: foo::Reader<'a>;
    type Builder<'a>: foo::Builder<'a>;
}
```
where `zap::traits::Owned` is defined as
...
```rust
struct MessageContainer<T> where T: zap::traits::Owned {
    message: zap::message::Builder<zap::message::HeapAllocator>,
    marker:  core::marker::PhantomData<T>,
}

impl <T> MessageContainer<T> where T: zap::traits::Owned {
...
In previous versions of zap-rust,
we needed to hoist the lifetime parameter `<'a>` to the top
of the declaration of `zap::traits::Owned`, like this:
...
```rust
struct MessageContainer<T> where T: for<'a> zap::traits::Owned<'a> {
    message: zap::message::Builder<zap::message::HeapAllocator>,
    marker:  core::marker::PhantomData<T>,
}
```
...
Sandstorm's [web publishing interface](https://github.com/sandstorm-io/sandstorm/blob/d8be67ad5f703bb73f5da4e607cce07106c2a0f1/src/sandstorm/web-publishing.zap#L24-L98):
...
The `zapc-rust`-generated code for the above schema will contain
...
structs to RPC system via the `zap_rpc::new_client()` function.
...
let client: blob_handle::Client = zap_rpc::new_client(blob_handle);
```

The issue is that once we call `zap_rpc::new_client()` we no longer have
...
a feature that has existed in zap-c++
for a long time, and as of today has been added to zap-rust.
If a `blob_handle::Client` is created via `CapabilityServerSet::new_client()`,
instead of `zap_rpc::new_client()`,
...
ZAP's
[packed codec](https://zap.org/encoding.html#packing)
is a way to reduce message size in exchange for
a minimal encoding/decoding cost.

Until recently, zap-rust only support the packed codec
via the synchronous `zap::serialize_packed` API; if you
...
[the addition](https://github.com/zap/zap-rust/pull/296)
of the `zap_futures::serialize_packed` module.
...
```
which can be plugged into `zap_rpc::twoparty::VatNetwork::new()`
