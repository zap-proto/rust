...
[#593](https://github.com/zap/zap-rust/pull/593).
...
Suppose we had a zap schema file `example.zap` declaring the following interface:
...
In previous versions, zapc-rust would generate a Rust trait with the following signature:
...
1. Rust users accustomed to `async/await` [found it annoying](https://github.com/zap/zap-rust/issues/168)
...
the generated method dispatch code. Therefore, zapc-rust no longer
supports generating code for Rust editions 2015 and 2018.

**Edit**: [zap-rust version 0.23]({{site.baseurl}}/2025/10/28/0.23-release.html)
