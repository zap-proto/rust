...
## v0.21.4
- Update minimum required version of zap base crate, to account for new
  usage of impl Clone for zap:: capability::Client.
...
- Fully-qualify `::zap::Word` in generated code.
- Add `zap --version` invocation before `zap compile`, for better error reporting.
...
## v0.15.0
- Support trait changes in zap::traits.
...
## v0.13.0
- Update to work without requiring "std" feature of zap base crate.
- Refer to `core` instead of `std` in generated code.
- Remove deprecated `ToClient` structs from generated code.

## v0.12.4
- Add `CompilerCommand.zap_executable()`.
- Remove obsolete `RustEdition` enum.

## v0.12.3
- Generate code for new zap::capability::FromServer trait.

## v0.12.2
- Add `parentModule` annotation to allow generated code to be included in a submodule.

## v0.12.1
- Add rust.zap file with `name` annotation for renaming items in generated Rust code.
...
- Use new const fn ::zap::word() instead of zap_word!() macro.
...
## v0.7.3
- `zapc -orust ./foo/bar/baz.zap` now correctly writes to `./foo/bar/baz_zap.rs` rather than
  just `./baz_zap.rs`. If you were depending on the old behavior you can use the `--src-prefix`
