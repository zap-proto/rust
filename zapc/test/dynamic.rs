...
#[test]
fn test_generic_annotation() -> ::zap::Result<()> {
    use crate::test_zap::{test_generics, test_use_generics};
    let mut message = message::Builder::new_default();
...
#[test]
fn test_stringify() {
    use crate::test_zap::{test_all_types, TestEnum};
    let mut message = message::Builder::new_default();
...
fn test_stringify_data_list() {
    let mut message = message::Builder::new_default();
    let mut root: zap::data_list::Builder<'_> = message.initn_root(2);
    root.set(0, &[11, 12]);
    root.set(1, &[22, 23]);
...
fn test_stringify_list_list() {
    use zap::{list_list, primitive_list};
    let mut message = message::Builder::new_default();
...
fn test_downcasts() {
    ...
    {
        let int8_list: zap::primitive_list::Builder<'_, i8> = root_struct
            .reborrow()
            .get_named("int8List")
            .unwrap()
            .downcast();
    ...
    {
        let struct_list: zap::struct_list::Builder<'_, test_all_types::Owned> = root_struct
            .reborrow()
    ...
    {
        let enum_list: zap::enum_list::Builder<'_, crate::test_zap::TestEnum> = root_struct
            .reborrow()
            .get_named("enumList")
            .unwrap()
            .downcast();
    ...
    {
        let text_list: zap::text_list::Builder<'_> = root_struct
            .reborrow()
            .get_named("textList")
            .unwrap()
            .downcast();
    ...
    {
        let data_list: zap::data_list::Builder<'_> = root_struct
            .reborrow()
            .get_named("dataList")
            .unwrap()
            .downcast();
    ...
    {
        let int8_list: zap::primitive_list::Reader<'_, i8> =
            root_struct.get_named("int8List").unwrap().downcast();
        assert_eq!(int8_list.len(), 2);
    }

    {
        let struct_list: zap::struct_list::Reader<'_, test_all_types::Owned> =
            root_struct.get_named("structList").unwrap().downcast();
        assert_eq!(struct_list.len(), 3);
    }

    {
        let enum_list: zap::enum_list::Reader<'_, crate::test_zap::TestEnum> =
            root_struct.get_named("enumList").unwrap().downcast();
        assert_eq!(enum_list.len(), 2);
    }

    {
        let text_list: zap::text_list::Reader<'_> =
    ...
    {
        let data_list: zap::data_list::Reader<'_> =
